...
############################## Variables ####################################
output_column = ["graduate"]
columns_to_drop = ["access","acad_year"]



################# Read data as pandas dataframe ############################
df=pd.read_csv('datasets/student_dropout_database.csv') #https://github.com/JKabathova/Students-Dropout-Prediction/blob/main/DBS.csv
cols = df.columns
...
df["temp"]=df["graduate"].astype(str)
...
model_binaryClassification = tf.keras.Sequential([
...
model = model_binaryClassification
...
y_pred = model.predict(test_inputs_array).flatten()
