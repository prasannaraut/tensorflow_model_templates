...
# slightly  modifying the question, taking bangalore weather data from following webpage and predict max temperature
# https://www.kaggle.com/datasets/hiteshsoneji/historical-weather-data-for-indian-cities?select=bengaluru.csv
...
df = pd.read_csv('datasets/bengaluru.csv')
print(df.head())
print(df.describe())
...
# create plot
df.set_index('date_time')["maxtempC"].plot()
#df["maxtempC"].plot()
plt.ylabel("maxtempC")
plt.title("Max Temperature time")
...
plt.plot(train_df['date_time'], train_df['maxtempC'], 'green',
label = 'Training Data')
plt.plot(valid_df['date_time'], valid_df['maxtempC'], 'blue',
label = 'Validation Data')
plt.title('Fixed Partitioning')
plt.xlabel('maxtempC')
plt.ylabel('maxtempC')
all_dates = np.concatenate([train_df['date_time'],
valid_df['date_time']])
...
x_valid = x_valid.reset_index()["maxtempC"]
