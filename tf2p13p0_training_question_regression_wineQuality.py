# source: https://github.com/ak-org/TFDevCert/blob/master/study_guide.md

# regression: red-wine quality
...
df=pd.read_csv('datasets/red_wind_quality.csv') #https://github.com/oluwole-packt/datasets/blob/main/salary_dataset.csv
...
values_train = np.array(train_labels['quality'])

onehot_encoder = OneHotEncoder(sparse=False, handle_unknown ='ignore')
values_train_ = values_train.reshape(len(values_train), 1)
onehot_enc= onehot_encoder.fit(values_train_)

train_labels_orig = train_labels
train_labels = onehot_enc.transform(values_train_)

values_test = np.array(test_labels['quality'])
values_test_ = values_test.reshape(len(values_test), 1)
test_labels_orig = test_labels
test_labels = onehot_enc.transform(values_test_)
...
preprocessing_complex = ColumnTransformer([
    ...
    ("log", log_pipeline, ["total sulfur dioxide", "density"]),
    ("geo", cluster_simil, ["fixed acidity"]),
...
model = tf.keras.Sequential([
    ...
    tf.keras.layers.Dense(512, activation='tanh', kernel_regularizer="l2"),
    tf.keras.layers.Dropout(0.5),
    tf.keras.layers.Dense(128, activation='tanh', kernel_regularizer="l2"),
...
# convert to one hot encoder like
y_pred_mod = []
for i in range(y_pred.shape[0]):
    ...
    #print(a_)
    #print('\n')
    y_pred_mod.append(a_)


print(y_pred_mod)

y_pred_quality = onehot_enc.inverse_transform(y_pred_mod)
true_quality = onehot_enc.inverse_transform(test_labels_array)
...
eval = confusion_matrix(true_quality, y_pred_quality)
print("######### Confusion Matrix #############")
print(eval)
print()
...
plt.figure()
plt.plot(true_quality)
plt.plot(y_pred_quality)
plt.show()
